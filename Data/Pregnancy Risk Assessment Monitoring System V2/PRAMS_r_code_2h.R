...
###################################################################
PRAMS <- read_sas("OMalley_2016_2018.sas7bdat") %>%
  transmute(##################################
            # TECHNICAL
            ##################################
            ID,
            # combine current upstate NY lable and New York City lable into one NY variable
...
# t1 <- PRAMS %>%
#   group_by(state, EVER_MAR, TYP_DDS) %>%
#   summarize(totes = n())
# 
# PRAMS%>%filter(state == "NY") %>%
#   group_by(EVER_MAR, DDSTRT) %>%
#   summarize( count = n())
      ...
      for (demo in demo_cat_list) {
        ...
        for (val in demo_val_list) {
          ...
          for (dir in c(1, 2)) {
            ...
            header <- paste(adj, geo, outcome, dir, demo, val, sep = "|")
